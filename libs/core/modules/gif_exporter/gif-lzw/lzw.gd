...
func decompress_lzw(code_stream_data: PackedByteArray, min_code_size: int, colors: PackedByteArray) -> PackedByteArray:
	...
	index_stream.append_array(code_table.get_idx(code).sequence)
	# set PREVCODE = CODE
	...
	while true:
		...
		var code_entry: CodeEntry = code_table.get_idx(code)
		if code_entry != null:  # if YES
			...
			code_table.add(code_table.get_idx(prevcode).add(k))
			# set PREVCODE = CODE
			prevcode = code
		else:  # if NO
			# let k be the first index of {PREVCODE}
			var prevcode_entry: CodeEntry = code_table.get_idx(prevcode)

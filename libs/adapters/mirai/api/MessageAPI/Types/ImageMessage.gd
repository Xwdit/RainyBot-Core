...
static func init(image:Image)->ImageMessage:
	if is_instance_valid(image):
		...
		if !err:
			...
			dic.path = f_path
			GuiManager.console_print_success("成功将Image图像实例缓存至文件并构造为图像消息: %s"% f_path,true,true)
			return ins
		else:
...
static func init_gif(gif_image:GifImage)->ImageMessage:
	if is_instance_valid(gif_image):
		...
		if !err:
			...
			var dic:Dictionary = ins.data_dic
			dic.path = f_path
			GuiManager.console_print_success("成功将GifImage图像实例缓存至文件并构造为图像消息: %s"% f_path,true,true)
			return ins
		else:
